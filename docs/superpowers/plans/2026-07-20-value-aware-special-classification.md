...
- Brilliant requires: best move or WDL expected-point loss `<= 2`; a mover-owned non-pawn piece worth at least `3` with opponent SEE gain `> 0`; centipawn score after `>= 50`; centipawn score before `< 97`.
- Great requires: played move equals Stockfish best move; centipawn pre-move score `< 99`; best-versus-second centipawn gap `>= 20`, with second-line WDL used only when second-line centipawn data is absent.
